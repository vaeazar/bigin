...
public enum SkillsRenew {
  ...
  SkillsRenew(String skillName, int skillMagicPoint, int skillDuration, String[] skillIncreaseName, String[] skillDecreaseName, double[] skillIncreaseValue, double[] skillDecreaseValue){
  ...
  public static int selectMagicPoint(String keys) {
    Optional<SkillsRenew> tempSkills = Arrays.stream(SkillsRenew.values())
        .filter(status -> status.skillName.equals(keys))
        .findFirst();
    return tempSkills.map(SkillsRenew::getSkillMagicPoint).orElse(-1);
  }
  ...
  public static int selectDuration(String keys) {
    Optional<SkillsRenew> tempSkills = Arrays.stream(SkillsRenew.values())
        .filter(status -> status.skillName.equals(keys))
        .findFirst();
    return tempSkills.map(SkillsRenew::getSkillDuration).orElse(-1);
  }

  public static SkillsRenew selection(String keys) {
    return Arrays.stream(SkillsRenew.values())
        .filter(status -> status.skillName.equals(keys))
        .findFirst()
        .orElse(FAIL);
  }
